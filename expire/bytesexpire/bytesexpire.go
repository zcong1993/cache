...
func (em *ExipreMap) Get(key interface{}) *[]byte {
	em.mu.RLock()
	defer em.mu.RUnlock()
	v, ok := em.store[key]
	if !ok {
...
func (em *ExipreMap) Set(k interface{}, val []byte, expire time.Duration) {
	em.mu.Lock()
	defer em.mu.Unlock()
	em.store[k] = Value{
		Val:       val,
...
func (em *ExipreMap) SetExpiredIn(k interface{}, val []byte, expiredIn time.Time) {
	em.mu.Lock()
	defer em.mu.Unlock()
	em.store[k] = Value{
		Val:       val,
...
func (em *ExipreMap) Update(k interface{}, newVal []byte) error {
	if !em.Has(k) {
