...
func (em *ExipreMap) Get(key interface{}) *string {
	em.mu.RLock()
	defer em.mu.RUnlock()
	v, ok := em.store[key]
	if !ok {
...
func (em *ExipreMap) Set(k interface{}, val string, expire time.Duration) {
	em.mu.Lock()
	defer em.mu.Unlock()
	em.store[k] = Value{
		Val:       val,
...
func (em *ExipreMap) SetExpiredIn(k interface{}, val string, expiredIn time.Time) {
	em.mu.Lock()
	defer em.mu.Unlock()
	em.store[k] = Value{
		Val:       val,
...
func (em *ExipreMap) Update(k interface{}, newVal string) error {
	if !em.Has(k) {
